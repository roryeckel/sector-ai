...
class BasicPoll(BaseModel):
    ...
    options: List[str] = Field(
        description=f"The {PollLimit.MIN_OPTION_NUMBER} or more options to choose from, must be less than {PollLimit.MAX_OPTION_NUMBER + 1} options."
...
async def poll_cmd(update: Update, context: SectorContext) -> None:
    ...
    if not (PollLimit.MIN_OPTION_NUMBER <= len(poll_response.options) <= PollLimit.MAX_OPTION_NUMBER):
        ...
        raise OutputParserException(
            f"Poll must have between {PollLimit.MIN_OPTION_NUMBER} and {PollLimit.MAX_OPTION_NUMBER} options."
        )
