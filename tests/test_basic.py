...
def test_package_versions():
    ...
    assert version.parse(langchain.__version__) >= version.parse("0.3.27")
    assert version.parse(telegram.__version__) >= version.parse("22.3")
    assert version.parse(pydantic.__version__) >= version.parse("2.11.7")
